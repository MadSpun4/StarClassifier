...
class KnowledgeEditorWindow(BaseWindow):
    ...
    def build_validation_page(self, parent):
        ...
        tk.Button(bottom, text='Сбросить к значениям по умолчанию', bg=COLORS['gray_btn'], bd=0, padx=16, pady=8, cursor='hand2', command=self._reset_defaults).pack(side='left')
    ...
    def _reset_defaults(self):
        if not messagebox.askyesno('Подтверждение', 'Сбросить текущую базу знаний к значениям по умолчанию?', parent=self):
            return
